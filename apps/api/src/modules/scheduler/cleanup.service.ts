...
export class CleanupService {
  ...
  /**
   * 每天凌晨 3 点清理旧贴文（需设置 POST_CLEANUP_ENABLED=true 才会执行）
   */
  @Cron(CronExpression.EVERY_DAY_AT_3AM)
  async cleanupOldPosts() {
    const enabled = this.configService.get<string>('POST_CLEANUP_ENABLED');
    if (enabled !== 'true') {
      this.logger.log('Post cleanup skipped (POST_CLEANUP_ENABLED is not set to true)');
      return;
    }
